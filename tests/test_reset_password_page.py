...
class TestResetPasswordPage:
    ...
    def test_get_reset_password_page(self, driver_both):
        reset_password_page = ResetPasswordPage(driver_both)
        reset_password_page.enter_and_click_reset_password()
        assert reset_password_page.get_current_url() == URL.FORGOT_PASSWORD_PAGE
    ...
    def test_input_email_and_reset(self, driver_both):
        reset_password_page = ResetPasswordPage(driver_both)
        reset_password_page.enter_and_click_reset_password()
        reset_password_page.set_email_and_click_restore_button()
        reset_password_page.wait_for_url_changes_restore()
        assert reset_password_page.get_current_url() == URL.RESET_PASSWORD_PAGE
    ...
    def test_active_password_field(self, driver_both):
        ...
        reset_password_page.set_email_and_click_restore_button()
        reset_password_page.wait_for_url_changes_restore()
        non_active = reset_password_page.get_input_status()
        reset_password_page.click_the_aye()
        active = reset_password_page.get_input_status()
